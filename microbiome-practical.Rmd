...
require(reshape2) # Reformating datasets
```
...
We will read in the species abundance data as provided by Palleja et al. Before doing so, it's worth understanding what their methods were for reaching these species abundances. 

Throughout this practical, there are <span style="background-color:#EFD31C">questions</span> about the material. Click on **Show answer** to reveal the answer - but do consider  having a go at thinking about it first!

<br>
<span style="background-color:#EFD31C">
**Question:** From the paper, how were operational taxonomic unit (OTU) abundances calculated? What is the advantage of this method compared to 16S rRNA marker gene screening? 
</span>
<details>
  <summary>**Show answer**</summary>
  <span style="background-color:#fff18c">
...
* The advantage of this method is that it uses multiple genes rather than a single marker gene (such as the 16S rRNA gene) so is more reliable for species differentiation. 
...
* Ensuring the same sequencing depth in each sample matters if you want to calculate diversity in a consistent way across samples. However, rarefying removes information on variance of species counts between samples. 
* Palleja et al. write that 'Samples were rarefied to 14 million inserts'. This means that each sample had 14 million paired-end reads (2 x 100 base pairs). In practice, this is very large compared to a lot of microbiome datasets and so rarefying is less of an issue here (apart from for very low abundance species).
</span>
</details>

<br>
We now read in the dataset. 
...
These are bacterial species.

So, we have a dataset where columns are samples and rows are bacterial species: an 'OTU table' like in the lecture. The entries are relative abundances for each species within a sample, which means that the sum of each column is 1 (total relative abundance within a sample is 1 by definition). We can check this.

```{r column_sums}
head(colSums(mOTUs))
```

If we look at the dataset in more detail, we can inspect the relative abundances for individual species.

```{r inspect_dataset}
head(mOTUs[,c("ERAS3_D0","ERAS3_D4")])
```

Here, `mOTUs[,c("ERAS3_Dag0","ERAS3_Dag4")]` selected two columns of the dataset from patient ERAS3. We then see that e.g. the species *Actinomyces odontolyticus* was present at a relative abundance of 0.0003927314 (=0.39%) on day 4 but was not present in the sample from day 0. 

<span style="background-color:#EFD31C">
**Question:** What is the smallest possible number of sequencing inserts (paired-end reads) from a species that would mean it could be present in the dataset (the 'limit-of-detection' or 'detection threshold')?
</span>
<details>
  <summary>**Show answer**</summary>
Any species detected must have at least 1 insert. 14 million inserts in total would mean that any species detected must be at an abundance of 1 in 14 million which is ~ 0.000007%. This is a lower bound. Note from the paper that 'at least 11.9 million read-pairs were mapped to IGC [integrated gene catalog of gut microbial genes]', so Palleja et al. write it would be 'on the order of 1 in 10 million'. This is quite low for a microbiome study; for example, with only 10,000 reads per sample (common in 16S rRNA gene studies) you would have a detection threshold of (1/10,000 * 100)% = 0.1%.  
</details>
...
We now use ``ggplot` to visualise the dataset. Each point is one sample. 
...
You might notice that the order of the days seems wrong in the legend. R has ordered them lexicographically. We can reorder them:
...
But we have many other choices. For example, principal components analysis (PCA) is common in microbiome studies. PCA has a different approach, where it finds the directions of greatest variation in the dataset and returns them in order: so PC1 is the axis made up of linear combinations of species abundances which captures the greatest variation, PC2 the second-greatest variation, and so on. 
...
<span style="background-color:#EFD31C">
**Question:** Why is this plot so different to the NMDS?
</span>
<details>
  <summary>**Show answer**</summary>
PCA finds the directions of greatest variation in the dataset. Here, the greatest variation is that immediately after antibiotics (D4) the gut microbiome is very different to all other timepoints. So that dominates the first two principal components and is shown in the visualization. Also, D4 samples can be very different to each other. This shows us that distances between points in NMDS are not necessarily indicative of fixed relationships between sets of samples. 
</details>

You should recall from the lecture that **an ordination is not a statistical test**. But here it is very clear that there is something to investigate further: from PCA we see very clearly that the gut microbiome is very perturbed at D4; from NMDS we see that it eventually returns to a similar state as baseline. (In the paper itself, they use a principal coordinates analysis or PCoA - which despite its name leads to a more similar plot to NMDS than PCA - there really are a huge number of choices here!)
...
<span style="background-color:#EFD31C">
**Question**: Find the definition of the Shannon index. What is the Shannon index of a sample with two species present at equal abundance?
</span>
<details>
  <summary>**Show answer**</summary>
Shannon index H = -sum(p_i log(p_i)) where p_i is proportional abundance of species i. (You can find this either on Wikipedia, or in the documentation for the `diversity` function which you can access with `?diversity`.)

Two species present at equal abundance in a sample means each has abundance p_1 = p_2 = 0.5. So it is
...
points(shannon[which(shannon<1 & richness>110)],
       richness[which(shannon<1 & richness>110)], pch=19, col='red')

```

I have highlighted one particular sample using `points` to add a red point to the plot. This is a useful example of the `which` function to select values which satisfy a particular set of conditions:  in this case, any sample which has a Shannon index less than 1, but a richness greater than 110. 

<span style="background-color:#EFD31C">
**Question:** What does a sample having high richness but low Shannon diversity (as indicated by the red point) mean about the distribution of species abundances? How could you look at this in more detail?
</span>
<details>
  <summary>**Show answer**</summary>
Many species are present (non-zero abundance), but their abundances are not very evenly distributed - the sample is dominated by a small number of species. We can find out which sample this is:
```{r low_shannon_diversity}
print(names(shannon[which(shannon<1 & richness>110)]))
head(sort(mOTUs[,"ERAS4_D4"], decreasing=TRUE))
```
This suggests that in this sample, a single species is present at 84.9%. The next-most abundant is at 3.0% abundance. So we have domination by a single species immediately after antibiotics. 
```{r identify_species}
head(rownames(mOTUs)[order(mOTUs[,"ERAS4_D4"], decreasing=TRUE)])
```

'motu_linkage_group_317' suggests it is not a known species from a database, but one identified from metagenomic sequencing alone - it does not have taxonomic identification given by Palleja et al. Later on, Palleja et al. write that 'For better interpretability of results, we removed mOTU linkage groups without taxonomic annotation' - that would include this mOTU, which could be important if present at such high relative abundance. 
</details>
...
We can look at this in more detail if we look at the richness - how many species were observed in each patient - over time. 
...
# Don't worry about interpreting the above function too much if it's not clear. Essentially we subtract the D0 richness for each patient from the actual richness of the sample. So D0 is defined to have a richness.norm=0. Negative richness.norm means the patient's gut microbiome on DX has a lower richness than on D0. 
...
So far we have largely considered the state of the gut microbiome as a whole i.e. using community measures such as diversity, or ordinations which use all species abundances. While this is good for showing large-scale patterns, much finer resolution analysis is possible.
...
<details>
  <summary>**Show answer**</summary>
The obvious outlier is the sample from D42 from patient ERAS12. This has a *B. bifidum* relative abundance of ~1%, markedly larger than all the others, and *B. bifidum* is not present at all D180. This could be a real pattern or not - the microbiome is very dynamic, but it seems surprising. 
</details>
...
<details>
  <summary>**Show answer**</summary>
There are **lots** of potential patterns here. One that jumps out at me is *B. dentium*. In ERAS6, ERAS9 and ERAS12 it **increases** in relative abundance after antibiotics by D8. This is surprisingly different from the other *Bifidobacterium* mOTUs. As its name suggests, it is apparently able to colonise the oral cavity as well, perhaps suggesting a different enviromental niche. 
</details>
